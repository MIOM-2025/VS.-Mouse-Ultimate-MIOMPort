...
void main()
{
    ...
    vec4 baseColor = flixel_texture2D(bitmap, uv); // get the actual colors
    
    // calculate the rays
    vec4 rayCol = crepuscular_rays(uv, vec2(0.5 + cos(time + .32 / 2) * .04, 0.3 + sin(time / 2) * .03));
    
    // add rays to the base color instead of replacing it
    gl_FragColor = baseColor + rayCol;
}